...
const notificationController = require('../../notification/src/controllers/notificationController');
...
exports.registerDoctor = async (req, res) => {
  try {
    ...
    await notificationController.notifyAdminDoctorRegistration(doctor);
...
exports.uploadProof = async (req, res) => {
  try {
    ...
    doctor.proof = { fileUrl, status: 'pending' };
    await doctor.save();

    // Notify doctor about proof verification status
    await notificationController.notifyDoctorVerification(doctor);

    res.status(200).json({ message: 'Proof uploaded successfully', doctor });
  } catch (error) {
    res.status(400).json({ message: error.message });
  }
};
...
exports.getUnverifiedDoctors = async (req, res) => {
  try {
    const unverifiedDoctors = await Doctor.find({ 'proof.status': 'pending' });
    res.status(200).json(unverifiedDoctors);
  } catch (error) {
    res.status(400).json({ message: error.message });
  }
};
...
exports.verifyDoctor = async (req, res) => {
  try {
    ...
    doctor.proof.status = status;
    doctor.isVerified = status === 'verified';
    await doctor.save();

    res.status(200).json({ message: `Doctor ${status} successfully`, doctor });
  } catch (error) {
    res.status(400).json({ message: error.message });
  }
};
